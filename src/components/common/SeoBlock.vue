...
<template>
	<div v-if="block.text || block.title">
		<div
			class="flex lg:gap-10 bg-gray-500 px-4 py-5 lg:py-12 lg:px-13 rounded-xl lg:rounded-3.5"
		>
			<div
				v-if="block.image"
				class="hidden lg:block w-[12.75rem] min-w-[12.75rem] aspect-square"
			>
				<img
			...
			<BlocksShowMoreWrapper :max-height-content="180">
				<template #default>
					<div>
						<h4 v-if="block.title" class="h1 mb-5">
							{{ block.title }}
						</h4>
				...
				<template #button="{ open, click }">
					<button
						@click="click"
						class="text-primary-blue flex gap-2 items-center"
					>
